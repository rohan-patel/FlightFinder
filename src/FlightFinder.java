...
public class FlightFinder {
	...
	public List<NavigableSet<Flight>> findFlights(int dayOfMonth, int month, int year, 
		...
		List<NavigableSet<Flight>> result = new ArrayList<NavigableSet<Flight>>();
        
        // Step 1: Construct ZonedDateTime objects to represent User-specified time interval when flights depart

        // Your code
        LocalTime depatureTimeStart = LocalTime.of(preferredDepartureStartHour, 00, 00);
				LocalDate departureDateStart = LocalDate.of(year, month, dayOfMonth);
				LocalDateTime departureDateTimeStart = LocalDateTime.of(departureDateStart, depatureTimeStart);
        ZonedDateTime zonedDepartureTimeStart = ZonedDateTime.of(departureDateTimeStart, ZoneId.of(departureCityTimeZone));
				// System.out.println(zonedDepartureTimeStart);
				
        LocalTime depatureTimeEnd = LocalTime.of(preferredDepartureEndHour, 00, 00);
				LocalDate departureDateEnd = LocalDate.of(year, month, dayOfMonth);
				LocalDateTime departureDateTimeEnd = LocalDateTime.of(departureDateEnd, depatureTimeEnd);
        ZonedDateTime zonedDepartureTimeEnd = ZonedDateTime.of(departureDateTimeEnd, ZoneId.of(departureCityTimeZone));
				// System.out.println(zonedDepartureTimeEnd);

        // Step 2: Find departing flights at departureCity
				// System.out.println(allFlights);
        List<Flight> allDepartingFlights = allFlights.get(departureCity);
				// System.out.println(allDepartingFlights.get(0).getDepartureCity());
        
        NavigableSet<Flight> departingflights = new TreeSet<Flight>();
				// departingflights
        
                   // Your code
                   // Tip: Methods like isAfter can be used to find flights in the specified user time interval
        allDepartingFlights.forEach((departingCityFlight) -> {
					// System.out.println(departingCityFlight);
					if(
						(departingCityFlight.getDepartureTime().isAfter(departureDateTimeStart)) && 
						(departingCityFlight.getDepartureTime().isBefore(departureDateTimeEnd)) &&
						(departingCityFlight.getArrivalCity() == arrivalCity)) {
							departingflights.add(departingCityFlight);
							// System.out.println(departingCityFlight);
						}
					});
					// System.out.println(departingflights.first());
        ...
        NavigableSet<Flight> connectingflights = new TreeSet<>();      
                    ...
                    // Your code  
				allConnectingFlights.forEach((connectingCityFlight) -> {
					if ((connectingCityFlight.getDepartureCity() == arrivalCity) &&
							(connectingCityFlight.getArrivalCity() == finalArrivalCity) &&
							(connectingCityFlight.getDepartureTime().isAfter(departingflights.first().getArrivalTime().plusHours(2)))) {
								// System.out.println(connectingCityFlight);
								connectingflights.add(connectingCityFlight);
							}
							// (connectingCityFlight.getDepartureTime())
				});
	...
	public static void main(String[] args) {
		...
		Flight f15 = new Flight(15, "15", "Emirates", "Dubai", "Mumbai",
				LocalDateTime.of(2017, 12, 20, 14, 0), LocalDateTime.of(2017, 12, 20, 14, 0));
